...
a = Analysis(
    ...
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],  # 一定要传入 hook.py 用于修改模块查找路径
...
exe = EXE(
    ...
    name='地价监测工具集',               # 程序的创口贴名字叫 hello
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    console=False,               # 运行时弹出终端窗口
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    icon='bin\\logo.ico',                    # 这里可以给 exe 加图标，如果你有图标文件的话
)
...
coll = COLLECT(
    ...
    upx=True,
    upx_exclude=[],
    name='地价监测工具集',   # 输出路径在 dist 文件夹里的 hello 文件夹
)